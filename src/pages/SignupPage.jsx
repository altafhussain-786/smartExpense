  ...
  return <div className="min-h-screen flex flex-col lg:flex-row bg-gray-100">
      <div className="relative w-full lg:w-1/2 bg-gradient-to-br from-orange-500 to-orange-600 text-white p-8 flex flex-col justify-between overflow-hidden">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-bold">{signupData.header.logo}</h2>
          <div className="text-sm">
            <span className="mr-2">{signupData.header.rightText}</span>
            <Link
              to={signupData.header.actionLink}
              className="font-semibold hover:underline transition"
            >
              {signupData.header.actionText}
            </Link>
          </div>
        </div>
        <div className="mt-10 space-y-4">
          ...
          <h1 className="text-4xl font-bold leading-tight">
            ...
            <span className="text-green-300">
              {signupData.leftPanel.heading.highlight}
            </span>
          </h1>
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-xs opacity-80">
                {signupData.leftPanel.statsCard.updated}
              </p>
            </div>
            <div className="text-right">
              <h3 className="text-lg font-bold">
                {signupData.leftPanel.statsCard.amount.currency}
                {signupData.leftPanel.statsCard.amount.value}
              </h3>
              <p className="text-xs text-green-300">
                {signupData.leftPanel.statsCard.growth.value}{" "}
                {signupData.leftPanel.statsCard.growth.label}
              </p>
            </div>
          </div>
          <div className="mt-4">
              ...
              <div
                ...
                style={{
                  width: `${(signupData.leftPanel.statsCard.progress.current /
                    signupData.leftPanel.statsCard.progress.total) *
                    100}%`,
                }}
              />
            </div>
            <div className="flex justify-between text-xs mt-2 opacity-80">
              <span>
                {signupData.leftPanel.statsCard.progress.labelLeft}
              </span>
              <span>
                {signupData.leftPanel.statsCard.progress.labelRight}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-md">
          ...
          <form className="space-y-4">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span>
                {signupData.rightPanel.form.checkbox.text}
              </span>
            </div>
            <button
              type="submit"
          ...
          <div className="flex items-center gap-3 my-6">
            ...
            <span className="text-gray-400 text-sm">
              {signupData.rightPanel.form.divider}
            </span>
            <div className="flex-1 h-px bg-gray-200"></div>
          </div>
          <div className="flex gap-4">
            {signupData.rightPanel.form.socialButtons.map(
              (socialBtn, index) => {
