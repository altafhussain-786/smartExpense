...
import { getRecentTransactions } from "../../../redux/features/dashboard/dashboardSlice"
import { recentTransactionsData } from "../../../data"

const RecentTransactions = () => {
    const dispatch = useDispatch()
    const { transactions, transactionsLoading } = useSelector(s => s.dashboard)
    ...
    const displayTransactions = transactions.length
        ? transactions.map((item, index) => ({
            ...item,
            icon: recentTransactionsData.transactions[index]?.icon
                || recentTransactionsData.transactions[0].icon
        }))
        : recentTransactionsData.transactions

    return (
        <div className="bg-white rounded-2xl shadow-md p-5 w-full transition-all duration-300 hover:shadow-lg">
